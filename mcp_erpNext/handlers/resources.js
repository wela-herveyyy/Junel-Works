...
export function registerResourceHandlers(server, erpnext, profile, doctypeCache) {
        ...
        resources: [
            {
                ...
                description: "Logged-in user profile (name, position, email, employee) — synced from ERPNext and local overrides",
            },
            {
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        if (!erpnext.isAuthenticated()) {
            throw new McpError(ErrorCode.InvalidRequest, `Not authenticated with ERPNext. ${AUTH_SETUP_HINT} Or configure API keys / ERPNEXT_CREDENTIALS_FILE.`);
        }
        let result;
        if (uri === "erpnext://DocTypes") {
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to fetch DocTypes: ${message}`);
            }
        }
        else {
            ...
            if (documentMatch) {
                ...
                try {
                    ...
                    const message = error instanceof Error ? error.message : "Unknown error";
                    throw new McpError(ErrorCode.InvalidRequest, `Failed to fetch ${doctype} ${name}: ${message}`);
                }
            }
        }
        if (!result) {
