...
import { mergeProfile } from "./types.js";
export async function syncProfileFromErpnext(client) {
    ...
    try {
        const users = await client.getDocList("User", { name: loggedUser }, ["name", "full_name", "email"], 1);
        const user = users[0];
        if (user) {
    ...
    try {
        const employees = await client.getDocList("Employee", { user_id: loggedUser }, [
            "name",
            "employee_name",
            "company",
            "department",
